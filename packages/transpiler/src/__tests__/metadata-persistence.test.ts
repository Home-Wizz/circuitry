import type { ConditionNode, FlowGraph, TriggerNode } from '@circuitry/shared';
import { isConditionNode, isTriggerNode } from '@circuitry/shared';
...
describe('Metadata Persistence', () => {
  ...
  describe('Native Strategy - Round Trip', () => {
    it('should preserve node positions when exporting and re-importing simple flow', async () => {
      ...
      expect(result.yaml).toContain('_circuitry_metadata');
...
mode: single
variables:
  # Intentionally the legacy _flode_metadata key (not _circuitry_metadata) --
  # this fixture is a regression test for the backward-compat read path in
  # YamlParser.ts's extractMetadata, proving an automation saved before the
  # Circuitry rebrand still gets its layout restored correctly.
  _flode_metadata:
    version: 1
    nodes:
...
action:
  - service: light.turn_on
variables:
  # Legacy key on purpose -- see the "should restore..." fixture above for why.
