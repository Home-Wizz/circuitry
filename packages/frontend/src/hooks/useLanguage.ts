...
export function useLanguage() {
  ...
  useEffect(() => {
    // Circuitry's own options-flow override (config_flow.py) takes precedence
    ...
    logger.debug('[Circuitry] useLanguage effect running', {
      hasHass: !!hass,
      languageOverride,
      hassLanguage,
      browserLanguage,
    ...
    if (targetLanguage !== i18n.language) {
      ...
      if (availableLanguages.includes(targetLanguage)) {
        logger.debug('[Circuitry] Changing language to:', targetLanguage);
        i18n.changeLanguage(targetLanguage);
      } else {
        logger.debug(
          `[Circuitry] Language '${targetLanguage}' not available, keeping '${i18n.language}'`
        );
      }
    }
