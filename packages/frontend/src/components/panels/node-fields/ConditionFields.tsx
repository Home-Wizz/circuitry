...
export function ConditionFields({ node, onChange, entities }: ConditionFieldsProps) {
  ...
  const handleConditionTypeChange = (newType: string) => {
    ...
    // legacy flat types' own fields (e.g. sun.after="sunrise" leaking into
    // time.after which expects HH:MM), but also native/device-condition
    // fields like `target`/`options` that live outside CONDITION_TYPE_FIELDS
    // entirely — see getAllConditionFieldNames' doc comment for the bug this
    // caused (a stale `options` object surviving a switch to "Time" and
    // getting rejected by HA's own config validator at save time).
    for (const fieldName of getAllConditionFieldNames()) {
  ...
  const renderConditionFields = () => {
    ...
    // `light.is_brightness`, `humidity.is_value`) use the shared
    // target/threshold field editor instead of config/conditionFields.ts's
  ...
  return (
    <>
      {/* Purpose-specific conditions (e.g. `light.is_brightness`) are only
          ever reached via recipes, not this flat condition-type list (which
          only has the legacy condition types) — showing it would just
          display an unmatched blank value, matching how TriggerFields.tsx
          hides its equivalent selector for dotted trigger types. */}
