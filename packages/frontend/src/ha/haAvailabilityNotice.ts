 ...
 * to Circuitry's own fallback on every affected field with no explanation.
 * Deduped per session — later failures are logged only (via `logger`, which
 * is itself gated behind Circuitry's debug flag).
 */
export function notifyHaComponentIssue(context: string): void {
  if (!hasNotified) {
    hasNotified = true;
    showWarningToast('Native HA-Komponenten nicht verfügbar', {
      description: 'Circuitry nutzt für einige Felder den eigenen Fallback statt HAs nativer UI.',
      duration: 6000,
    });
  }
