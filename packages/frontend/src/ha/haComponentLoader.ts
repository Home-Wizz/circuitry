...
async function probe(name: string): Promise<boolean> {
  ...
  try {
    ...
    if (!loadCardHelpers) {
      logger.debug('[Circuitry] window.loadCardHelpers unavailable — not running inside HA?');
      return false;
    }
 ...
 * within the timeout — callers should fall back to Circuitry's own component.
 */
