...
export const HANDLED_PROPERTIES = {
  ...
  trigger: [
    ...
    'minutes',
    'seconds',
    // Tag trigger properties
    'tag_id',
    // Geolocation trigger properties
    'source',
    // Persistent notification trigger properties
    'notification_id',
    'update_type',
    // Conversation (sentence) trigger properties
    'command',
    ...
    'type',
    'subtype',
    // Purpose-specific trigger properties (handled by NativeTriggerFields —
    // e.g. `light.turned_on`'s target: {...} / options: { behavior }
    'target',
    'options',
    // Universal trigger fields (id/enabled handled directly by
    // PropertyPanel.tsx; variables handled by the "Advanced" section in
    // TriggerFields.tsx — see TriggerAdvancedFields.tsx)
    'variables',
  ],
