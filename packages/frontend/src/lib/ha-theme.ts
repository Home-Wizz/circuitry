 ...
 * table.
 *
 * `--trigger`/`--condition`/`--action` are Circuitry's own node-type colors, but
 ...
 * neutral scale) that some native HA components Circuitry embeds read directly
 ...
 * `HA_THEME_TOKENS` set above, these don't reliably inherit into Circuitry's
 ...
 * resolve incorrectly from inside Circuitry's shadow tree (dropdown items
...
/**
 * Circuitry's own light/dark override — independent from Home Assistant's
 ...
 *
 * `override` lets Circuitry force HA's base light/dark palette regardless of the
 ...
 * token — native HA web components Circuitry embeds (entity/area/label pickers,
 ...
 * normal (non-portaled) descendants of Circuitry's shadow tree, showing up as a
...
export function applyHaTheme(
  ...
  for (const [localVar, token] of Object.entries(HA_THEME_TOKENS)) {
    ...
    // Resolve against `target` itself — see toHslTriplet's doc comment —
    // so a theme value that references a further `var(...)` (e.g. Vision
    // OS-style themes building rgba() colors from an inner rgb-triplet
    // variable) resolves in the exact cascade it'll actually be used in.
    const triplet = toHslTriplet(rawValue, target);
    if (triplet) {
    ...
    // Capture only the first entry's value per haVar (same one actually written
    // to `--${token.haVar}` below). Several distinct local tokens intentionally
    // share the same haVar — e.g. `warning-foreground`/`trigger-foreground` both
    // point at `primary-text-color` but hardcode '#141414' in both light AND
    // dark mode (fixed dark text for contrast on a bright warning/trigger
    // badge, not the theme's actual text color). Capturing on every iteration
    // let those later entries silently clobber haVarValues['primary-text-color']
    // with their fixed light-mode value even while in dark mode, which then fed
    // the wrong color into HA_WEBAWESOME_TEXT_TOKENS below (--wa-color-text-normal
    // ended up permanently '#141414', making unselected ha-select/ha-dropdown-item
