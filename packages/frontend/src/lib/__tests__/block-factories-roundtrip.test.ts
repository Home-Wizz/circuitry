...
describe('block-factories: structural output', () => {
  ...
  it('createIfElseBlock: a condition wired to two placeholder If/Else branches', () => {
    const { nodes, edges, entryNodeIds } = createIfElseBlock(0, 0);
    const condition = nodes.find((n) => n.type === 'condition');
    const actions = nodes.filter((n) => n.type === 'action');
    expect(condition).toBeDefined();
    expect(actions).toHaveLength(2);
    expect(nodes.every((n) => n.data._blockKey === 'if_else')).toBe(true);
    expect(entryNodeIds).toEqual([condition!.id]);

    // condition --(true)--> if-branch, condition --(false)--> else-branch
    ...
    expect(actions.map((a) => a.id)).toContain(trueEdge?.target);
    expect(actions.map((a) => a.id)).toContain(falseEdge?.target);
    expect(trueEdge?.target).not.toBe(falseEdge?.target);
  });
  ...
  it('createRepeatUntilBlock: action forward to condition, false-loop-back', () => {
    const { nodes, edges, entryNodeIds } = createRepeatUntilBlock(0, 0);
    const condition = nodes.find((n) => n.type === 'condition');
    const action = nodes.find((n) => n.type === 'action');
    expect(condition).toBeDefined();
    expect(action).toBeDefined();
    expect(condition?.data._blockKey).toBe('repeat_until');

    // Layout: action sits to the LEFT of the condition, same as repeat_while.
    expect(action!.position.x).toBeLessThan(condition!.position.x);

    // action --(forward)--> condition  (body runs before the check, unlike while)
    const forwardEdge = edges.find((e) => e.source === action!.id && e.target === condition!.id);
    expect(forwardEdge).toBeDefined();
    expect(forwardEdge?.type).not.toBe('loop-back');

    // condition --(false, loop-back)--> action
    const loopBack = edges.find((e) => e.type === 'loop-back');
    expect(loopBack?.source).toBe(condition!.id);
    expect(loopBack?.target).toBe(action!.id);
    expect(loopBack?.sourceHandle).toBe('false');

    // The loop body runs first — a connection into this block must wire to
    // the action, NOT the condition (the reverse of repeat_while).
    expect(entryNodeIds).toEqual([action!.id]);
  });
...
describe('block-factories: transpile round-trip', () => {
  ...
  it('repeat_until → repeat block with until condition', () => {
    const { nodes, edges } = createRepeatUntilBlock(200, 150);
    const action = nodes.find((n) => n.type === 'action')!;
    const trig = triggerNode('trigger_ru');
    const result = transpile([trig, ...nodes], [...edges, edge(trig.id, action.id)]);

    expect(result.success).toBe(true);
    expect(result.yaml).toContain('repeat');
    expect(result.yaml).toContain('until');
    expect(result.yaml).not.toContain('service: undefined');
  });
