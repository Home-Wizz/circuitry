...
import { FlowTranspiler, transpiler } from '@circuitry/transpiler';
  ...
  yamlFiles.forEach((filename) => {
    it(`should preserve data integrity for ${filename}`, async () => {
      ...
      if (finalYaml) {
        ...
        // Metadata validation (Circuitry variables should be present)
        const variables = finalConfig.variables as
          | Record<string, Record<string, unknown>>
          | undefined;
        expect(variables?._circuitry_metadata).toBeDefined();
        // All exports use version 1 metadata (positions only - node data and edges are in the YAML structure)
        expect(variables?._circuitry_metadata.version).toBe(1);
        // Strategy can be 'native' or 'state-machine' depending on complexity
        expect(['native', 'state-machine']).toContain(variables?._circuitry_metadata.strategy);
      }
