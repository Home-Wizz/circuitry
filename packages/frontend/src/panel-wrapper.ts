/**
 * Home Assistant panel entry point for Circuitry.
 *
 * Mounts the React app directly into a Shadow DOM on this custom element
 * (no iframe) so Circuitry shares HA's document — required for native HA web
 ...
 *
 * The shadow root keeps Circuitry's Tailwind styles from leaking into HA's own
...
interface CircuitryPanelInfo {
  config?: {
    language?: string;
  };
}

class CircuitryPanelWrapper extends HTMLElement {
  private _hass: HomeAssistant | undefined;
  private _narrow = false;
  private _languageOverride: string | undefined;
  private appHandle: CircuitryAppHandle | null = null;
  ...
  /** Set by `ha-panel-custom` — carries `config` from `panel_custom.async_register_panel`, i.e. Circuitry's own options-flow settings. */
  set panel(value: CircuitryPanelInfo | undefined) {
  ...
  connectedCallback() {
    ...
    this.appHandle = mountCircuitryApp(appRoot, {
...
// Register the custom element. Tag name must exactly match what panel.py
// registers via panel_custom.async_register_panel's webcomponent_name
// (`f"{DOMAIN}-panel"`, i.e. `circuitry-panel`) — HA creates that exact
// tag to host this panel, and an unregistered tag never upgrades, so nothing
// would render (just HA's own background showing through an inert element).
if (!customElements.get('circuitry-panel')) {
  customElements.define('circuitry-panel', CircuitryPanelWrapper);
}
