...
All notable changes to Circuitry are documented here. Entries before the
rebrand below were written under this project's previous name — FLODE —
...
This project began as a fork of [C.A.F.E.](https://github.com/FezVrasta/cafe-hass)
by Federico Zivolo, was developed for several months under the name **FLODE**
(with a brief internal-only rename during a private working-copy phase), and
is now rebranded **Circuitry** as its own distinct identity going forward. Every
change below this entry happened under one of those earlier names; nothing
about the automation transpiler, the canvas, or the underlying HA integration
changed functionally as part of the rename itself — this is a naming and
identity change, not a rewrite.

### Changed
- **HA integration domain**: reverted to `circuitry` (from the domain used
  during an earlier internal-only phase of this project). This means
  upgrading from FLODE is not a silent in-place update — the old integration
  must be removed and Circuitry added fresh (config entries are keyed by
  domain in Home Assistant). See the README's "Upgrading from FLODE"
  section.
- **Panel route**: reverted to `/circuitry` (from the path used during an
  earlier internal-only phase of this project, itself preceded by `/flode`).
  Any dashboard deep-link buttons built against the old path need updating —
  see the README's Deep Links section.
- **HA event bus event**: `flode_automation_saved` → `circuitry_automation_saved`.
- **npm workspace scope**: reverted to `@circuitry/*` (from the scope used
  during an earlier internal-only phase of this project) across all three
  packages (`shared`, `frontend`, `transpiler`).
- **In-browser draft storage** (IndexedDB key `flode-flow-storage` →
  `circuitry-flow-storage`): only affects unsaved, in-progress canvas drafts,
  not automations already saved to Home Assistant.

### Fixed
- **`reportImportIssue`'s HA service call still targeted the old `flode`
  service domain** after the Python integration's own domain had already
  been renamed — every lossy-import Repair issue was silently failing to
  file (caught and only best-effort logged) until this was caught during the
  rebrand sweep. Now calls the `circuitry` domain, matching
  `custom_components/circuitry/const.py`'s `DOMAIN`.

### Compatibility
- **Automations saved by FLODE (including during its brief internal-only
  working-copy phase) or the original C.A.F.E. keep their saved canvas
  layout.** The visual-layout metadata Circuitry writes
  into an automation's `variables:` block is now `_circuitry_metadata`, but
  the parser still reads the older `_flode_metadata` and `_cafe_metadata`
  keys as a fallback — opening an automation saved under any earlier name
  restores its node positions exactly as before, with no manual conversion
  step. The very next save from Circuitry writes the current key.

---

## [1.6.0] — 2026-08-09 — Trigger Picker Rebuild

See `docs/trigger-picker-rebuild.md` for the full technical write-up.

### Added
- **Trigger node picker now mirrors Home Assistant's own "Add trigger" dialog** — a "By target" tab (a real Home > Area > Device > Entity tree) and a "By type" tab (browse by domain/device-class: Light, Cover, Battery, Motion, ...), replacing the old flat platform dropdown. The "By type" catalog is cross-verified against real HA source (each domain's `trigger.py` platform or `device_trigger.py` device-class table) rather than guessed, and covers battery, button, climate, cover, fan, light, lock, media player, motion, occupancy, temperature, timer, vacuum, and more.
- **"By type" is now type-first**, matching HA's own dialog order: pick a category and its triggers show directly in a results panel below; the trigger commits immediately and entities are assigned afterward via the normal entity field, instead of requiring entities to be picked before any trigger options appear.
- **"Change trigger" back button** — an already-configured trigger can now return to the By target/By type picker with the previous tab and selection intact, instead of the only way back being to delete the node and start over.
- **New integration icon**, in both the brand assets (this project's `custom_components/<domain>/brand/` folder, under the domain name used at the time) and the Home Assistant sidebar icon (previously `mdi:graph-outline`, now `mdi:transit-connection-variant`) — these are two separate icon references in HA and both needed updating.

### Fixed
- **HA's `{subtype}`/`{device_name}` placeholder templates weren't substituted** in device-automation trigger labels (e.g. ZHA's `"{subtype}" pressed` rendered with the literal, unsubstituted text), and some labels duplicated the same text twice (e.g. "Device offline: device_offline").
- **New nodes could spawn off-screen, and the canvas's fit-to-view zoom could pin every automation to a fixed tiny scale** — a leftover viewport-reset effect was firing after React Flow's own fit-to-view and overriding it every time.
- **Cover's "By type" entry listed 11 near-duplicate sub-categories**, six of them sharing the identical generic label "Cover" — collapsed into one flat, correctly-labeled entry (Awning/Blind/Curtain/Damper/Door/Garage door/Gate/Shade/Shutter/Window), matching real HA's own dialog, which lists "Cover" once and shows every device class's triggers together.
